    ...
    <div className="mt-8 grid grid-cols-2 p-4 border gap-8 rounded-3xl shadow-md shadow-gray-300 ">
      <div className="flex relative group">
        <Image
          ...
          src={place.photos[currIndex]}
        ></Image>

        <div className="absolute top-[50%] text-red rounded-full p-1  left-2 pt-2">
          <button
            className=" bg-gray-500 group-hover:block hidden bg-opacity-20"
            onClick={prevPhoto}
          >
            <svg
      ...
      <div className="grid grid-rows-2">
        <div className="grid grid-cols-3 gap-1 mt-4 mx-4 border-b p-4 h-16">
          <h1>@{place.owner}</h1>
        ...
        <div className="border rounded-2xl p-4 h-30">
          <h1 className="text-2xl">{place.title}</h1>
          <h2 className="underline">{place.address}</h2>

          <h1 className="mt-4 flex"> @{place.owner}:  {place.description} </h1>
        </div>
      </div>
    </div>
  );
